   ...
   /* if (user.styleid = null) {
    ...
    async function countdownimg(senderId, user) {
      ...
      botly.send({
        id: senderId,
        message: {
          text: "⏰ انتهت ✅ مدة الانتظار 💤 يمكنك ارسال صورة جديدة 🌇",
          quick_replies: quickReplies,
        },
      });
    }
    ...
    if (message.message.text) {
      ...
      const forbiddenWords =["قضيب", "Penis", "عاري", "Naked", "جنس", "Sex", "سكس", "Sexual", "غير لائق", "Inappropriate", "إباحي", "Pornographic", "فاحش", "Obscene","Suggestive", "شائن", "Shameful", "منحرف", "Perverted", "فاضح", "Indecent", "سافل", "Vulgar", "فاسق", "Immoral", "مبتذل", "Indelicate", "مشين", "Disgusting", "فاسد", "Corrupt","Twisted", "مسيء","Offensive","gay","lesbian","غاي","ليزبيان","pussy","ass","مهبل","مؤخرة","زب","سوة","طيز","ترمة","تناسلي"];
       // ["ass"];
...
const data = new URLSearchParams();
    ...
    */
          ...
          async function generatePromptViaImage() {
            const url = "https://cognise.art/api/generatepromptviaimage";
            const boundary = "6bccbfc2-829a-4e43-b045-9b3dfed4d117";
            const imagePath = `users_image/${senderId}.png`; // Update the path to your image

            const imageStats = fs.statSync(imagePath);
            const imageLength = imageStats.size;

            const form = new FormData();
            form.append("img", fs.createReadStream(imagePath), {
              // filename: '1703335771125.png',
              contentType: "image/png",
              knownLength: imageLength, // Update with your image's actual length
            });
            ...
            try {
              ...
              const response = await axios.post(url, form, {
                headers: {
                  ...
                  "Content-Length": form.getLengthSync(), // Get the content length from form-data
              ...
              console.log("Response:", response.data.data.caption);
 ...
 botly.sendText({
          ...
          async function downloadImage(url) {
            try {
              ...
              const imagePath = `users_image/${senderId}.png`; // File path/name where the image will be saved

              fs.writeFileSync(imagePath, response.data);
              // Save the image data to 'image' file

              //console.log("Image downloaded and updated at:", imagePath);
              countdownimg(senderId, user);

              generatePromptViaImage();
...
const onPostBack = async (senderId, message, postback) => {
  ...
  if (
    message &&
    message.postback &&
    message.postback.title.startsWith("الاعدادات") || message &&
      message.message &&
      message.message.text &&
